...
public:
    ...
    // Returns true if the opeartor matches the character at s[r.start],
    // modifying r in the case of match
    virtual bool match(const string &s, Range &r) const = 0;
};


/* A class representing an operation to match a specific character. */
class MatchChar : public RegexOperator {
    // Character to match with this operation
    char matchChar;
...
/* A class representing an operation to match any character. */
...
class MatchFromSubset : public RegexOperator {
    // Collection of characters to match from with this operation
    string characters;
...
class ExcludeFromSubset : public RegexOperator {
    // Collection of characters to exclude from with this operation
    string characters;
